...
static const int MAX_INIT_SILENCE_BYTES = 176400;
...
int main(int argc, char ** argv)
{
    ...
    if (argc < 2)
    {
        std::fprintf(stderr, "usage: aopsf_helper <file.psf|minipsf>\n");
        return 2;
    }
    ...
    if (data.empty())
    {
        std::fprintf(stderr, "aopsf: unable to read input file\n");
        return 3;
    }

    if (data.size() < 4 || std::memcmp(data.data(), "PSF\001", 4) != 0)
    {
        std::fprintf(stderr, "aopsf: only PSF1 is supported by this helper\n");
        return 4;
    }

    if (psf_start(reinterpret_cast<uint8_t *>(data.data()), static_cast<uint32_t>(data.size())) != AO_SUCCESS)
    {
        std::fprintf(stderr, "aopsf: psf_start failed\n");
        return 5;
    }

    const int result = psf_execute(write_pcm);
    psf_stop();
