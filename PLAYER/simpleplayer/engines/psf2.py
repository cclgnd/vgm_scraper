...
class Psf2EngineUnavailable(EngineError):
    pass


class Psf2Engine:
    display_name = "PSF2 / Audio Overload"
    supported_extensions = {".psf2", ".minipsf2"}
    ...
    def is_supported(path: Path) -> bool:
        return path.suffix.lower() in Psf2Engine.supported_extensions
    ...
    def open(self, path: Path, track: int = 0) -> list[TrackInfo]:
        if not path.exists():
            raise Psf2EngineUnavailable(f"File does not exist: {path}")
        if not self.is_supported(path):
            raise Psf2EngineUnavailable(f"Unsupported PSF2 format: {path.suffix or '(none)'}")
        ...
        with self._lock:
            ...
            self._rendered_frames = 0
            self._start_helper()
            return [TrackInfo(index=0, title=path.stem, system="Sony PlayStation 2")]
    ...
    def tracks(self) -> list[TrackInfo]:
        if not self._path:
            return []
        return [TrackInfo(index=0, title=self._path.stem, system="Sony PlayStation 2")]

    def start_track(self, index: int) -> None:
        if index != 0:
            raise Psf2EngineUnavailable("PSF2 files expose one streamed track per file.")
        if self._path:
            self.open(self._path)

    def seek_ms(self, position_ms: int) -> None:
        if position_ms != 0:
            raise Psf2EngineUnavailable("PSF2 seeking is not implemented yet; restart is supported.")
        if self._path:
            self.open(self._path)
    ...
    def _start_helper(self) -> None:
        if not self._path:
            raise Psf2EngineUnavailable("No PSF2 file loaded.")
        if not self._helper.exists():
            raise Psf2EngineUnavailable(
                f"PSF2 helper is not built: {self._helper}. Run native/aopsf2/build_aopsf2.py first."
            )
    ...
    def _helper_path() -> Path:
        return Path(__file__).resolve().parents[2] / "engines" / "aopsf2" / "aopsf2_helper.exe"
