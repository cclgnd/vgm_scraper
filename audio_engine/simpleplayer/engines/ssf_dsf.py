...
class SsfDsfEngineUnavailable(EngineError):
    pass


class SsfDsfEngine:
    display_name = "SSF / DSF / Highly Theoretical"
    supported_extensions = {".ssf", ".minissf", ".dsf", ".minidsf"}
    ...
    def is_supported(path: Path) -> bool:
        return path.suffix.lower() in SsfDsfEngine.supported_extensions
    ...
    def open(self, path: Path, track: int = 0) -> list[TrackInfo]:
        if not path.exists():
            raise SsfDsfEngineUnavailable(f"File does not exist: {path}")
        if not self.is_supported(path):
            raise SsfDsfEngineUnavailable(f"Unsupported format: {path.suffix or '(none)'}")
        ...
        with self._lock:
            ...
            self._rendered_frames = 0
            self._start_helper()
            system = "Sega Saturn" if path.suffix.lower() in {".ssf", ".minissf"} else "Sega Dreamcast"
            return [TrackInfo(index=0, title=path.stem, system=system)]
    ...
    def tracks(self) -> list[TrackInfo]:
        if not self._path:
            return []
        system = "Sega Saturn" if self._path.suffix.lower() in {".ssf", ".minissf"} else "Sega Dreamcast"
        return [TrackInfo(index=0, title=self._path.stem, system=system)]

    def start_track(self, index: int) -> None:
        if index != 0:
            raise SsfDsfEngineUnavailable("SSF/DSF files expose one streamed track per file.")
        if self._path:
            self.open(self._path)

    def seek_ms(self, position_ms: int) -> None:
        if position_ms != 0:
            raise SsfDsfEngineUnavailable("SSF/DSF seeking is not implemented yet; restart is supported.")
        if self._path:
            self.open(self._path)
    ...
    def _start_helper(self) -> None:
        if not self._path:
            raise SsfDsfEngineUnavailable("No SSF/DSF file loaded.")
        if not self._helper.exists():
            raise SsfDsfEngineUnavailable(
                f"SSF/DSF helper is not built: {self._helper}. Run native/htssf/build_aoht.py first."
            )
    ...
    def _helper_path() -> Path:
        return Path(__file__).resolve().parents[2] / "engines" / "aoht" / "aoht_helper.exe"
