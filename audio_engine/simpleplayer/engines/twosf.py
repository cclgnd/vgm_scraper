...
class TwosfEngineUnavailable(EngineError):
    pass


class TwosfEngine:
    display_name = "Nintendo DS (2SF)"
    supported_extensions = {".2sf", ".mini2sf"}
    ...
    def is_supported(path: Path) -> bool:
        return path.suffix.lower() in TwosfEngine.supported_extensions
    ...
    def open(self, path: Path, track: int = 0) -> list[TrackInfo]:
        if not path.exists():
            raise TwosfEngineUnavailable(f"File does not exist: {path}")
        if not self.is_supported(path):
            raise TwosfEngineUnavailable(f"Unsupported format: {path.suffix or '(none)'}")
        ...
        with self._lock:
            ...
            self._rendered_frames = 0
            self._start_helper()
            return [TrackInfo(index=0, title=path.stem, system="Nintendo DS")]
    ...
    def tracks(self) -> list[TrackInfo]:
        if not self._path:
            return []
        return [TrackInfo(index=0, title=self._path.stem, system="Nintendo DS")]

    def start_track(self, index: int) -> None:
        if index != 0:
            raise TwosfEngineUnavailable("2SF files expose one streamed track per file.")
        if self._path:
            self.open(self._path)

    def seek_ms(self, position_ms: int) -> None:
        if position_ms != 0:
            raise TwosfEngineUnavailable("2SF seeking is not implemented yet; restart is supported.")
        if self._path:
            self.open(self._path)
    ...
    def _start_helper(self) -> None:
        if not self._path:
            raise TwosfEngineUnavailable("No 2SF file loaded.")
        if not self._helper.exists():
            raise TwosfEngineUnavailable(
                f"2SF helper is not built: {self._helper}. Run native/vio2sf/build_vio2sf.py first."
            )
        ...
        # Give the helper time to initialize and skip initial silence
        import time
        time.sleep(0.1)
    ...
    def _helper_path() -> Path:
        return Path(__file__).resolve().parent / "vio2sf" / "vio2sf_helper.exe"
