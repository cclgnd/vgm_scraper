...
class UsfEngineUnavailable(EngineError):
    pass


class UsfEngine:
    display_name = "Nintendo 64 (USF)"
    supported_extensions = {".usf", ".miniusf"}
    ...
    def is_supported(path: Path) -> bool:
        return path.suffix.lower() in UsfEngine.supported_extensions
    ...
    def open(self, path: Path, track: int = 0) -> list[TrackInfo]:
        if not path.exists():
            raise UsfEngineUnavailable(f"File does not exist: {path}")
        if not self.is_supported(path):
            raise UsfEngineUnavailable(f"Unsupported format: {path.suffix or '(none)'}")
        ...
        with self._lock:
            ...
            self._rendered_frames = 0
            self._start_helper()
            return [TrackInfo(index=0, title=path.stem, system="Nintendo 64")]
    ...
    def tracks(self) -> list[TrackInfo]:
        if not self._path:
            return []
        return [TrackInfo(index=0, title=self._path.stem, system="Nintendo 64")]

    def start_track(self, index: int) -> None:
        if index != 0:
            raise UsfEngineUnavailable("USF files expose one streamed track per file.")
        if self._path:
            self.open(self._path)

    def seek_ms(self, position_ms: int) -> None:
        if position_ms != 0:
            raise UsfEngineUnavailable("USF seeking is not implemented yet; restart is supported.")
        if self._path:
            self.open(self._path)
    ...
    def _start_helper(self) -> None:
        if not self._path:
            raise UsfEngineUnavailable("No USF file loaded.")
        if not self._helper.exists():
            raise UsfEngineUnavailable(
                f"USF helper is not built: {self._helper}. Run native/aousf/build_aousf.py first."
            )
    ...
    def _helper_path() -> Path:
        return Path(__file__).resolve().parent / "aousf" / "aousf_helper.exe"
