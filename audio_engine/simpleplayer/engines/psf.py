...
class PsfEngineUnavailable(EngineError):
    pass


class PsfEngine:
    display_name = "PSF1 / Audio Overload"
    supported_extensions = {".psf", ".minipsf"}
    ...
    def is_supported(path: Path) -> bool:
        return path.suffix.lower() in PsfEngine.supported_extensions
    ...
    def open(self, path: Path, track: int = 0) -> list[TrackInfo]:
        if not path.exists():
            raise PsfEngineUnavailable(f"File does not exist: {path}")
        if not self.is_supported(path):
            raise PsfEngineUnavailable(f"Unsupported PSF format: {path.suffix or '(none)'}")
        ...
        with self._lock:
            ...
            self._rendered_frames = 0
            self._start_helper()
            return [TrackInfo(index=0, title=path.stem, system="Sony PlayStation")]
    ...
    def tracks(self) -> list[TrackInfo]:
        if not self._path:
            return []
        return [TrackInfo(index=0, title=self._path.stem, system="Sony PlayStation")]

    def start_track(self, index: int) -> None:
        if index != 0:
            raise PsfEngineUnavailable("PSF files expose one streamed track per file.")
        if self._path:
            self.open(self._path)

    def seek_ms(self, position_ms: int) -> None:
        if position_ms != 0:
            raise PsfEngineUnavailable("PSF seeking is not implemented yet; restart is supported.")
        if self._path:
            self.open(self._path)
    ...
    def _start_helper(self) -> None:
        if not self._path:
            raise PsfEngineUnavailable("No PSF file loaded.")
        if not self._helper.exists():
            raise PsfEngineUnavailable(
                f"PSF helper is not built: {self._helper}. Run native/aopsf/build_aopsf.py first."
            )
    ...
    def _helper_path() -> Path:
        return Path(__file__).resolve().parents[2] / "engines" / "aopsf" / "aopsf_helper.exe"
